...
test('Change position when clicking in the viewport', async ({ page }) => {
  ...
  await expect(page.locator('css=skraafoto-viewport')).toContainText('koordinat 722102 Ø, 6178846 N set fra nord.')
  await expect(page.getByTitle('Billede af området omkring koordinat 722102 Ø, 6178846 N set fra øst.')).toBeDefined()
})